...
|---:|---:|---:|---:|
| 4 | 43.2ms | 11.6ms | 73.1% |
| 8 | 87.1ms | 12.2ms | 86.0% |
| 16 | 173.2ms | 23.9ms | 86.2% |

相对上一版 4 路并发，8/16 Skill 又分别从约 `23.1/45.8ms` 降至 `12.2/23.9ms`，继续降低约 47%。按当前复用连接后约 184ms 的公网 RTT 只计算网络等待下界，16 Skill 会从旧串行的 16 个波次约 `2.94s`，降到 8 路并发的两个波次约 `0.37s`；这是 RTT 推算，不冒充真实任务端到端结果。
