...
| 证据 | 结果 | 能证明什么 |
|---|---|---|
| 3 组、360 Job 扩大真实任务 A/B | 原始架构基线最终闭环成功率 66.7%；稳定性组与最终性能组分别为 180/180，最终性能组的 52 个首次失败全部自动恢复 | 结构化失败证据 + 有界恢复在固定任务集中的收益，以及上下文按需检索 + Session 复用未降低闭环成功率；180/180 的 95% Wilson 区间下界约 97.9%，不能宣称未知任务普遍 100% |
...
首次扩大评测得到的 95% 已废弃：复盘发现 CSV Verifier 的 JSON 字符串转义错误会拒绝正确实现。新增“正确实现必须被 Verifier 接受”的回归测试后先稳定复现红灯，再修正 Verifier；同时把 `Error: base` 这类不可执行的单词错误升级为带期望契约的结构化验证证据。没有放宽验收、增加第三次重试或人工干预。原失败集 20 Job 定向复测全部通过；随后 3 组扩大 A/B 中，`guard/reuse` 两臂分别达到 180/180。

简历可写：

> 构建任务级不可变上下文快照与只读按需查询接口，仅默认注入当前任务及工作流概要，并结合单 Issue Session 复用，减少长链路重复上下文传输；扩大真实任务 A/B 中 Token 消耗降低 27%、Issue 中位耗时降低 13%，复杂 Workflow 在本评测集的最终闭环成功率由 66.7% 提升至 100%。
...
- Multica Skill 首次下载保持“每个 Skill 独立请求、独立校验、独立缓存”，由串行改为单任务最多 8 路并发，并用 Daemon 级 16 路总上限约束多个任务的聚合流量；单个失败仍不会清空其他成功缓存。
...
| 未缓存 Skill 数 | 旧串行 | 8 路有界并发 | 降低 |
|---:|---:|---:|---:|
| 4 | 43.5ms | 11.8ms | 73.0% |
| 8 | 87.6ms | 12.3ms | 85.9% |
| 16 | 175.5ms | 24.3ms | 86.2% |
| 32 | 350.3ms | 46.5ms | 86.7% |
| 64 | 698.0ms | 92.0ms | 86.8% |

64 Skill 专项每种方案执行 5 轮、每轮 10 次；完整解析和 Daemon 总限流测试又连续执行 10 次，全部通过。生产边界仍为单任务最多 8 路、单 Daemon 最多 16 路，避免多任务同时启动时形成无界请求风暴。按当前复用连接后约 184ms 的公网 RTT 只计算网络等待下界，64 Skill 会从旧串行的 64 个波次降到 8 路并发的 8 个波次；这是 RTT 推算，不冒充真实任务端到端结果。
