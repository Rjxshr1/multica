...
| 证据 | 结果 | 能证明什么 |
|---|---|---|
| 180 Job 扩大真实任务 A/B | 性能组相对稳定性组总 Token 降低 12.7%、Issue 中位耗时降低 23.3%；最终闭环成功率 `95.0% vs 96.7%`，60 个精确配对任务只相差 1 个，未观察到统计显著下降 | 上下文按需检索 + Session 复用整体策略在真实模型调用中的收益；不能宣称成功率严格零影响，也不能把 Token 降幅全部单独归因于 Context Gateway |
...
> 构建任务级不可变上下文快照与只读按需查询接口，仅默认注入当前任务及工作流概要，并结合单 Issue Session 复用，减少长链路重复上下文传输；扩大真实任务 A/B 中 Token 消耗降低 13%、Issue 中位耗时降低 23%，复杂 Workflow 闭环成功率保持 95%，未观察到统计显著下降。
