...
| 证据 | 结果 | 能证明什么 |
|---|---|---|
| 修正后 180 Job 扩大真实任务 A/B | 原始架构最终闭环成功率 66.7%，稳定性组与性能组均为 60/60；性能组相对稳定性组总 Token 降低 26.9%、Issue 中位耗时降低 12.9% | 结构化失败证据 + 有界恢复在固定任务集中的收益，以及上下文按需检索 + Session 复用未降低闭环成功率；60/60 的 95% 区间下界为 94.0%，不能宣称普遍 100% |
...
首次扩大评测得到的 95% 已废弃：复盘发现 CSV Verifier 的 JSON 字符串转义错误会拒绝正确实现。新增“正确实现必须被 Verifier 接受”的回归测试后先稳定复现红灯，再修正 Verifier；同时把 `Error: base` 这类不可执行的单词错误升级为带期望契约的结构化验证证据。没有放宽验收、增加第三次重试或人工干预。原失败集 20 Job 定向复测全部通过，随后修正后的完整评测得到上述 60/60。
